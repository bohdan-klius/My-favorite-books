<template>
  <form @submit.prevent="submitForm">
    <div class="form-control" :class="{invalid: !nameOfBook.isValid}">
      ...
      <input type="text" id="nameOfBook" v-model="nameOfBook.val" @blur="clearValidity('nameOfBook')" />
    ...
    <div class="form-control" :class="{invalid: !theAuthorOfTheBook.isValid}">
      ...
      <input type="text" id="theAuthorOfTheBook" v-model="theAuthorOfTheBook.val" @blur="clearValidity('theAuthorOfTheBook')" />
      <p v-if="!theAuthorOfTheBook.isValid">The author of the book must not be empty</p>
    </div>
    <div class="form-control" :class="{invalid: !description.isValid}">
      <label for="description">Note</label>
      <textarea name="description" id="description" rows="5" v-model="description.val" @blur="clearValidity('description')"></textarea>
...
<script>
  export default {
    ...
    mounted() {
      console.log(this.selectedBook.nameOfBook);
      console.log('NAME OF BOOK' , this.nameOfBook);
    },
    methods: {
      clearValidity(input) {
      ...
      onValidationInit() {
        this.nameOfBook.val = this.selectedBook.nameOfBook;
        this.theAuthorOfTheBook.val = this.selectedBook.theAuthorOfTheBook;
        this.description.val = this.selectedBook.description;
        this.status.val = this.selectedBook.status;
      },
      submitForm() {
        this.validateForm();
